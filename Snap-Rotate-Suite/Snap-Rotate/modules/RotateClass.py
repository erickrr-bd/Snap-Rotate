					...
					while True:
						...
						if now.hour == int(time_execution_rotate[0]) and now.minute == int(time_execution_rotate[1]):
							...
							if not len(list_indices_not_writeables) == 0:
								elastic.createRepositoryFS(conn_es, name_repository, snap_rotate_conf['path_repositories'] + '/' + name_repository)
								self.utils.createSnapRotateLog("\nRepository created: " + name_repository, 1)
								print("\nRepository created: " + name_repository)
								message_creation_end_repository = telegram.getMessageEndCreationRepository(name_repository, snap_rotate_conf['path_repositories'] + '/' + name_repository)
								telegram.sendTelegramAlert(self.utils.decryptAES(snap_rotate_conf['telegram_chat_id']).decode('utf-8'), self.utils.decryptAES(snap_rotate_conf['telegram_bot_token']).decode('utf-8'), message_creation_end_repository)
								for index in list_indices_not_writeables:
									...
									telegram.sendTelegramAlert(self.utils.decryptAES(snap_rotate_conf['telegram_chat_id']).decode('utf-8'), self.utils.decryptAES(snap_rotate_conf['telegram_bot_token']).decode('utf-8'), message_creation_start_snapshot)
									...
									telegram.sendTelegramAlert(self.utils.decryptAES(snap_rotate_conf['telegram_chat_id']).decode('utf-8'), self.utils.decryptAES(snap_rotate_conf['telegram_bot_token']).decode('utf-8'), message_creation_end_snapshot)
									if snap_rotate_conf['is_delete_index'] == True:
										...
										if not conn_es.indices.exists(index = index):
											...
											telegram.sendTelegramAlert(self.utils.decryptAES(snap_rotate_conf['telegram_chat_id']).decode('utf-8'), self.utils.decryptAES(snap_rotate_conf['telegram_bot_token']).decode('utf-8'), message_delete_index)
								last_day_month = monthrange(now.year, now.month)[1]
								if now.day == last_day_month:
									if snap_rotate_conf['is_compress_repository'] == True:
										print("\nCompression of the repository has started...")
										path_repository = snap_rotate_conf['path_repositories'] + '/' + name_repository + '.tar.gz'
										with open_tf(path_repository, "w:gz") as tar_file:
											tar_file.add(snap_rotate_conf['path_repositories'] + '/' + name_repository)
										if path.exists(path_repository):
											...
											telegram.sendTelegramAlert(self.utils.decryptAES(snap_rotate_conf['telegram_chat_id']).decode('utf-8'), self.utils.decryptAES(snap_rotate_conf['telegram_bot_token']).decode('utf-8'), message_compress_repository)
							else:
